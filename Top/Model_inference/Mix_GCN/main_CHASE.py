...
class Processor():
    ...
    def __init__(self, arg):
        ...
        #     self.model = DDP(self.model, device_ids=[self.output_device])
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                # dataset=Feeder(transform=train_transforms, **self.arg.train_feeder_args),
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            if label.dim() > 1:  # 如果 label 是多维的
                label = label.view(-1)  # 转换为一维
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    label = label.long().cuda(self.output_device)
                    output = self.model(data)
                    # label = label.unsqueeze(1)  # 将其形状变为 [128, 1],3d pose
                    # print(label.shape)  # 输出 label 的形状
