...
class Feeder(Dataset):
    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            #self.label = np.where(npz_data['y_train'] > 0)[0]
            self.sample_name = ['train_' + str(i) for i in range(len(self.data))]
        elif self.split == 'test':
            self.data = npz_data['x_test']
            self.label = npz_data['y_test']
            #self.label = np.where(npz_data['y_test'] > 0)[0]
    ...
    def __getitem__(self, index):
        ...
        if(valid_frame_num == 0):
            ...
            return torch.zeros((3, 120, 17, 2)), label, index
            #elif  self.window_size == 128:
                #return torch.zeros((3, 128, 17, 2)), label, index
            #elif  self.window_size == 32:
                #return torch.zeros((3, 32, 17, 2)), label, index
