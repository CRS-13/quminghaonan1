...
def valid_crop_resize(data_numpy,valid_frame_num,p_interval,window):
    ...
    if len(p_interval) == 1:
        ...
        if valid_size - cropped_length < 0:
            bias = 0  # 如果裁剪长度超出有效大小，从头开始
        else:
        ...
        # 处理全零数组的情况
        if np.all(data == 0) and cropped_length > 0:
            # print("data_numpy 是全零数组，保留一些数据")
            data = data_numpy[:, :10, :, :]  # 保留前10个时间步

    # 确保数据不为空
    if data.shape[1] == 0:
        # print("裁剪后数据为空，使用默认填充值")
        data = np.zeros((C, 10, V, M))  # 使用适当的填充值

    # resize
    data = torch.tensor(data, dtype=torch.float)
    data = data.permute(0, 2, 3, 1).contiguous().view(C * V * M, data.shape[1])
    data = data[None, None, :, :]  # 增加两个维度以适配插值函数
