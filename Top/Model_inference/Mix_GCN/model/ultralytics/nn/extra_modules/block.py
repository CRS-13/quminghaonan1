...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C2f_AdditiveBlock', 'C2f_AdditiveBlock_CGLU', 'CSP_MSCB', 'EUCB', 'C2f_MSMHSA_CGLU', 'CSP_PMSFA', 'C2f_MogaBlock']
...
class EUCB(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1):
        ...
        self.gate = nn.Conv2d(
            in_channels=self.out_channels, out_channels=self.out_channels, kernel_size=1)

    def forward(self, x, y):
        y = self.up_dwc(x)
        y = self.channel_shuffle(y, self.in_channels)
        y = self.pwc(x)
